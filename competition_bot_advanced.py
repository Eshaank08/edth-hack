#!/usr/bin/env python3
"""
Competition bot using the advanced CNN model.
Memory-efficient inference for real-time predictions.
"""
...
MODEL_PATH = Path(__file__).parent / "models" / "best_model_advanced.pt"
TEMP_AUDIO_PATH = Path(__file__).parent / "temp_challenge.wav"


class CompetitionBotAdvanced:
    """Advanced competition bot with CNN model."""
    ...
    def _load_model(self) -> tuple[nn.Module, AdvancedAudioFeatureExtractor]:
        ...
        model = get_model(model_type, num_classes=len(CLASSES), dropout=0.4)
    ...
    def classify_audio(self, audio_path: Path) -> str:
        ...
        try:
            ...
            # Run inference
            with torch.no_grad():
                outputs = self.model(mel_spec, mfcc, stats)
    ...
    def run(self, poll_interval: float = 2.0) -> None:
        ...
        logger.info("="*60)
        logger.info("🚀 Advanced Competition Bot Starting...")
...
def main() -> None:
    """Main entry point."""
    bot = CompetitionBotAdvanced(auth_token=AUTH_TOKEN, model_path=MODEL_PATH)
